...
class VideoDescriptor(VideoFields, VideoTranscriptsMixin, VideoStudioViewHandlers,
    ...
    def validate(self):
        ...
        for lang, transcript in self.transcripts.items():
            if not transcript:
                no_transcript_lang.append([label for code, label in settings.ALL_LANGUAGES if code == lang][0])
        ...
        if no_transcript_lang:
            ...
            validation.set_summary(
                StudioValidationMessage(
                    ...
                    ungettext(
                        'There is no transcript file associated with {lang} language.',
                        'There are no transcript files associated with {lang} languages.',
