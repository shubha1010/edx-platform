...
from openedx.core.djangoapps.external_auth.views import SHIBBOLETH_DOMAIN_PREFIX
...
def register_user(request, extra_context=None):
    ...
    if context.get("extauth_domain", '').startswith(SHIBBOLETH_DOMAIN_PREFIX):
...
def login_user(request, error=""):  # pylint: disable=too-many-statements,unused-argument
    ...
    if settings.FEATURES.get('AUTH_USE_SHIB') and user:
        try:
            ...
            if eamap.external_domain.startswith(SHIBBOLETH_DOMAIN_PREFIX):
                return JsonResponse({
                    "success": False,
...
def create_account_with_params(request, params):
    ...
    * The user-facing text is rather unfriendly (e.g. "Username must be a
    ...
    tos_required = (
        ...
        not eamap.external_domain.startswith(SHIBBOLETH_DOMAIN_PREFIX)
    )
